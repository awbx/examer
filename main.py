...
class Examer:
    ...
    def register(self):
        ...
        if token_el and exam_el:
            ...
            res = self.sess.post(self.build_url(
                f"{exam_el.attrs.get('data-url')[1:]}/exams_users", BASE_URL), data=data)
        elif not exam_el:
...
try:
    if not forty_two_user or not forty_two_pass:
        print("Missing the credentials!", file=sys.stderr)
        exit(1)
